...
export const selectBillById = createSelector(
  ...
  (bills: Bill[], billId: string) => bills.find((bill: Bill) => bill.id === billId)
);

// Get bills by status
export const selectBillsByStatus = createSelector(
  [selectAllBills, (state: RootState, status: 'pending' | 'paid' | 'cancelled') => status],
  (bills: Bill[], status: 'pending' | 'paid' | 'cancelled') => bills.filter((bill: Bill) => bill.status === status)
);
...
export const selectBillsByOwner = createSelector(
  ...
  (bills: Bill[], ownerId: string) => bills.filter((bill: Bill) => bill.owner.user.id === ownerId)
);
...
export const selectBillsByParticipant = createSelector(
  ...
  (bills: Bill[], userId: string) => bills.filter((bill: Bill) => 
...
export const selectRecentBills = createSelector(
  ...
  (bills: Bill[]) => {
    ...
    return bills.filter((bill: Bill) => 
      new Date(bill.creationDate) >= thirtyDaysAgo
    ).sort((a: Bill, b: Bill) => 
...
export const selectBillsSortedByDate = createSelector(
  [selectAllBills],
  (bills: Bill[]) => [...bills].sort((a: Bill, b: Bill) => 
    new Date(b.creationDate).getTime() - new Date(a.creationDate).getTime()
  )
);
...
export const selectBillsSortedByAmount = createSelector(
  [selectAllBills],
  (bills: Bill[]) => [...bills].sort((a: Bill, b: Bill) => b.totalAmount - a.totalAmount)
);
...
export const selectBillsStats = createSelector(
  ...
  (bills: Bill[]) => {
    ...
    const pendingBills = bills.filter((bill: Bill) => bill.status === 'pending').length;
    const paidBills = bills.filter((bill: Bill) => bill.status === 'paid').length;
    const cancelledBills = bills.filter((bill: Bill) => bill.status === 'cancelled').length;
    
    const totalAmount = bills.reduce((sum: number, bill: Bill) => sum + bill.totalAmount, 0);
...
export const selectUserBillsStats = createSelector(
  ...
  (bills: Bill[], userId: string) => {
    ...
    const ownedBills = bills.filter((bill: Bill) => bill.owner.user.id === userId);
    ...
    const amountOwedToUser = ownedBills.reduce((sum: number, bill: Bill) => {
      const participantsAmount = bill.participants.reduce((partSum: number, participant) => 
        partSum + participant.amountToPay, 0
      );
      return sum + participantsAmount;
    }, 0);

    // Calculate amounts user owes (bills they participate in but don't own)
    const amountUserOwes = participatedBills.reduce((sum: number, bill: Bill) => {
      const userParticipant = bill.participants.find(p => p.user.id === userId);
...
export const selectBillsBySearch = createSelector(
  ...
  (bills: Bill[], searchTerm: string) => {
    ...
    return bills.filter((bill: Bill) => 
      ...
      (bill.description && bill.description.toLowerCase().includes(lowercaseSearch))
    );
  }
);
