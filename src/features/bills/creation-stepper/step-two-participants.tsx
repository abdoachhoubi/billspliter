...
import { Users, Plus, X, Edit3, UserPlus } from 'lucide-react-native';
...
export const StepTwoParticipants: React.FC<StepTwoParticipantsProps> = ({
  ...
  const handleUpdateParticipantAmount = (index: number, amount: number) => {
    ...
    if (isCurrentUser) {
      ...
      const totalBillAmount = parseFloat(totalAmount || '0');
      const otherParticipantsTotal = participants.reduce(
        (sum, p) => sum + p.amount,
        0
      );

      const maxForCurrentUser =
        splitType === 'percentage'
      ...
      const participantIndex = participants.findIndex(
        p => p.contact.id === participant.contact.id
      );
      ...
      >
        ...
        >
          ...
          <TouchableOpacity
            ...
            style={{
              ...
              borderRadius: BORDER_RADIUS.md,
              backgroundColor:
                remainingValue > 0 ? COLORS.premium : COLORS.border,
            }}
          >
            <Plus
              size={18}
              color={
                remainingValue > 0 ? COLORS.background : COLORS.textSecondary
              }
            />
            <Text
              style={{
        ...
        <View
          style={{
            ...
            borderLeftWidth: 4,
            borderLeftColor:
              remainingValue >= 0 ? COLORS.premium : COLORS.error,
          }}
        >
          <Text
            style={{
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {splitType === 'percentage' && (
                      ...
                      >
                        ≈ $
                        {((participant.amount / 100) * totalBillAmount).toFixed(
                          2
                        )}
                      </Text>
                    )}
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      setEditingParticipant(
                        editingParticipant === index ? null : index
                      )
                    }
                    style={{
                      padding: SPACING.sm,
                ...
                {editingParticipant === index && (
                  ...
                  >
                    ...
                    <Slider
                      ...
                      onValueChange={value =>
                        handleUpdateParticipantAmount(index, value)
                      }
        ...
        {participants.filter(p => p.contact.id !== currentUser.id).length ===
          0 && (
          <View
            style={{
              paddingVertical: SPACING.lg,
