...
import {
  ...
  StepTwoParticipants,
  StepThreeSummary,
} from '../features/bills/creation-stepper';
        ...
        <React.Fragment key={index}>
          <View
            style={{
              ...
              borderRadius: 16,
              backgroundColor:
                index <= currentStep ? COLORS.premium : COLORS.border,
              alignItems: 'center',
              justifyContent: 'center',
            }}
          >
...
export const CreateBillScreen: React.FC<CreateBillScreenProps> = ({
  ...
  const canProceedToStep2 =
    title.trim() && totalAmount && parseFloat(totalAmount) > 0;
  ...
  const canProceedToStep3 = useMemo(() => {
    ...
    const allocatedValue = otherParticipants.reduce(
      (sum, p) => sum + p.amount,
      0
    );
    ...
    if (splitType === 'percentage') {
      ...
      return (
        allocatedValue > 0 && allocatedValue <= parseFloat(totalAmount || '0')
      );
    }
    ...
    try {
      ...
      Alert.alert(
        ...
        error instanceof Error
          ? error.message
          : t('create_bill.error_create_bill'),
        [{ text: 'OK' }]
      );
    }
  };
  ...
  const handleNext = () => {
    if (currentStep === 0 && !canProceedToStep2) {
      Alert.alert(
        ...
        'Please fill in the bill title and total amount to continue.'
      );
      return;
    }
  ...
  const getStepTitle = () => {
    switch (currentStep) {
      case 0:
        return t('create_bill.step_1_title');
      case 1:
        return t('create_bill.step_2_title');
      case 2:
        return t('create_bill.step_3_title');
      default:
        return '';
    }
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.background }}>
      ...
      >
        ...
        >
          <TouchableOpacity
            onPress={() =>
              currentStep === 0 ? navigation.goBack() : handlePrevious()
            }
            style={{
              padding: SPACING.xs,
